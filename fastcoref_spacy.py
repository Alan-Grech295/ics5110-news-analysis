...
# Retrieved from https://github.com/shon-otmazgin/fastcoref/issues/59#issuecomment-2395181112
class PatchedLingMessCoref(LingMessCoref):
...
class FastCorefResolver:
    ...
    def __init__(
        ...
        if model_architecture == "FCoref":
            self.coref_model = PatchedFCoref(model_name_or_path=model_path, device=device, nlp=nlp, enable_progress_bar=enable_progress_bar)
        elif model_architecture == "LingMessCoref":
            self.coref_model = PatchedLingMessCoref(model_name_or_path=model_path, device=device, nlp=nlp, enable_progress_bar=enable_progress_bar)
